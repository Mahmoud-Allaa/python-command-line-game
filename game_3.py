...
def print_pause(message):
    print(message)
    sleep(.4)  # Add a delay of 1 second between messages
...
def star_messages():
    ...
    print_pause(
        "In your hand you hold your trusty (but not very effective), magic wand.")
...
def house(the_enemy, the_wand, score):
    ...
    print_pause(
        f"You are about to knock when the door opens and out steps a {the_enemy}.")
...
def defend(the_enemy, the_wand, score):
    ...
    if choice == '1':
        if the_wand == 'Wand of Ogoroth':
            new_score += 100
            print_pause(
                f'As the {the_enemy} moves to cast a spell, you raise your new Wand of Ogoroth.')
            print_pause(
                f'The Wand of Ogoroth shines brightly in your hand as you brace yourself for the spell.')
            print_pause(
                f'But the {the_enemy} takes one look at your shiny new wand and runs away!')
            print_pause(
